 ...
 * @desc   API接口类
 */
interface EspApiInterface {
    /**
     * 发送消息
     */
    @GET("/get")
    suspend fun getMessage(@Query("message") message: String): String
    ...
    suspend fun collectOutsideArticle(
        @Field("title") title: String, @Field("author") author: String, @Field("link") link: String
    ): BaseResponse<Any>?
    ...
    suspend fun cancelMyCollect(
        @Path("id") id: Int, @Field("originId") originId: Int = -1
    ): BaseResponse<Any>?
    ...
    suspend fun searchResult(
        @Path("page") page: Int, @Field("k") keyWord: String
    ): BaseResponse<ArticleList>?
